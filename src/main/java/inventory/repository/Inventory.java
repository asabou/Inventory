...
public class Inventory {
    ...
    public Product lookupProduct(String searchItem) {
        Product productFound = null; //1
        Product productReturned = null;
        boolean isFound = false;
        for (Product p : products) { //2
            if (p.getName().contains(searchItem) || (p.getProductId()+"").equals(searchItem)) {//3
                productFound = p; //4
                isFound = true;
                break;
            }//5
        ...
        if (isFound) { //6
            productReturned = productFound; //7
        } else
        if (!isFound) { //8
            productReturned = new Product(0, null, 0, 0, 0, 0, null); //9
        }
        return productReturned; //10
    }
